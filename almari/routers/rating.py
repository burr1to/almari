...
router = APIRouter(prefix="/rating", tags=["Rating And Review"])


@router.post(
    "/{id}", status_code=status.HTTP_201_CREATED, response_model=schema.RatingOut
)
def rating(
    id: int,
    rating: schema.RatingBase,
    db: Session = Depends(database.get_db),
    current_user: int = Depends(oauth2.get_current_user),
):
    new_rating = models.Rating(
        owner_id=current_user.id, product_id=id, **(rating.dict())
    )
    db.add(new_rating)
    db.commit()
...
@router.put(
    "/{rating_id}", status_code=status.HTTP_200_OK, response_model=schema.RatingOut
)
def update_rating(
    updated_item: schema.RatingBase,
    rating_id: int,
    db: Session = Depends(database.get_db),
    current_user: int = Depends(oauth2.get_current_user),
):
